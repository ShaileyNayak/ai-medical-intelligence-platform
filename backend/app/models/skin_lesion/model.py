"""Skin lesion module — binary Malignant / Benign (stretch goal)."""
...
from app.models.skin_lesion.gradcam import GradCAM, get_target_layer
...
class SkinLesionModel:
    """Model loading + inference for skin lesion (optional / stretch)."""
    ...
    def _load(self) -> None:
        path = resolve_weight_path(
            settings.skin_lesion_model_path,
            "skin_lesion/best_model.pth",
        )
        ...
        if loaded:
            logger.info("skin_lesion weights loaded from %s", path)
        else:
            logger.warning("skin_lesion checkpoint missing at %s — untrained head", path)
